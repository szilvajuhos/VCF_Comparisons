...
void CompareVCFs::CompareVariants(ProcessVCF metavcf, int nof_vcfs){
    ...
    ofile << "Number of Common indels      " << '\t' << NofCommonKnownSNPs + NofCommonNovelSNPs << '\t' << n1 << '\t' << n2 << endl
         << "Number of Common Known indels" << '\t' << NofCommonKnownSNPs << '\t' << n3 << '\t' << n4 << endl
         << "Number of Common Novel indels" << '\t' << NofCommonNovelSNPs << '\t' << n5 << '\t' << n6 << endl
         << "Number of Known indels only detected in first"  << '\t' << NofUnique1_KnownSNPs << endl
         << "Number of Known indels only detected in second" << '\t' << NofUnique2_KnownSNPs << endl
         << "Number of Novel indels only detected in first"  << '\t' << NofUnique1_NovelSNPs << endl
         << "Number of Novel indels only detected in second" << '\t' << NofUnique2_NovelSNPs << endl;

   cout << "Number of Common indels      " << '\t' << NofCommonKnownSNPs + NofCommonNovelSNPs << '\t' << n1 << '\t' << n2 << endl
        << "Number of Common Known indels" << '\t' << NofCommonKnownSNPs << '\t' << n3 << '\t' << n4 << endl
        << "Number of Common Novel indels" << '\t' << NofCommonNovelSNPs << '\t' << n5 << '\t' << n6 << endl
        << "Number of Known indels only detected in first"  << '\t' << NofUnique1_KnownSNPs << endl
        << "Number of Known indels only detected in second" << '\t' << NofUnique2_KnownSNPs << endl
